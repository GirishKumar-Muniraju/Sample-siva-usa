...
describe('Outage User Feedback Manager Tests', () => {
    let userFeedbackManager: OutageUserFeedbackManager;
    let userFeedbackDaoStub: sinon.SinonStubbedInstance<UserFeedbackDao<Outage>>;

    beforeEach(() => {
        userFeedbackDaoStub = sinon.createStubInstance(UserFeedbackDao);
        userFeedbackManager = new OutageUserFeedbackManager();
    ...
    it('should log an error and throw if an error occurs', async () => {
        ...
        try {
            ...
            sinon.assert.calledWith(loggerSpy, 'An error occurred to store user feedback for Outage: ', error);
        }
    });
});
